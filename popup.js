chrome.tabs.getSelected(null, function(tab) {
	...
	chrome.tabs.sendMessage(tab.id, {greeting: "hello"}, function(response) {
		...
		if (response) {
			...
			var trackset = {};
			$.getJSON('http://ws.spotify.com/search/1/track.json?page=1&q=artist:' + encodeURIComponent(artist), function (data) {
				...
				$.map(data.tracks, function(item){
					...
					var uri = item.href.replace('spotify:track:','');
					trackset[trackname] = uri;	// Using trackname as key will remove tracks with duplicate names
				});
		
				// Spotify will return up to 100 tracks, limit to 20 tracks
				var uriset = '';
				var i = 0;
				$.each(trackset, function (name,uri){
					uriset += uri + ',';
					i += 1;
					if (i >= 20){
						return false;
				...
				var playerurl = 'https://embed.spotify.com/?uri=spotify:trackset:' + encodeURIComponent(artist) + ':' + uriset;	
